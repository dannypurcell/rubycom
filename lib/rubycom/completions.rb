  ...
  module Completions
    ...
    def tab_complete(base, arguments, command_plugin)
      ...
      if args.nil? || args.empty?
        matches = command_plugin.get_top_level_commands(base).map { |sym| sym.to_s }
      elsif args.length == 1
        matches = command_plugin.get_top_level_commands(base).map { |sym| sym.to_s }.select { |word| !word.match(/^#{args[0]}/).nil? }
